 ...
 *
 * This script reads chunks exported from parsing and generates embeddings
 * using our TypeScript EmbeddingService with jinaai/jina-embeddings-v2-base-code.
 *
 * Usage:
 *   tsx tools/generate-embeddings.ts <chunks-file> <output-file>
 */
...
async function main() {
  ...
  if (args.length < 2) {
    console.error('Usage: tsx tools/generate-embeddings.ts <chunks-file> <output-file>');
    console.error('');
    console.error('Example:');
    console.error('  tsx tools/generate-embeddings.ts chunks.json embeddings.json');
    process.exit(1);
  }
